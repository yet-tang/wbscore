#!/usr/bin/env python3
"""
Train laya v3 — fixed version that removes data leakage.

Key fix vs v2: do NOT use quality_score_proxy as a state feature.
That field is what we're trying to predict, so using it as input causes the
model to be useless at inference (where we don't have it).

Training labels (target_distribution) still use quality_score_proxy
(that's the ground truth), but state features only use observable data
(rating, feedbacks, photos, options, etc.).
"""
import json
import os
import sys
...
WEIGHTS_DIR = WORKSPACE / "checkpoints" / "wb_laya_v3"
LOG_FILE = WORKSPACE / "training_log_v3.json"
...
def main():
    ...
    log = {"loss": [], "n_samples": len(sequences), "epochs": epochs,
           "encoder": encoder_name, "started_at": time.time(), "version": "v3 (no leak)"}
